...
module HarzardUnit(
    ...
    always@(*) begin
        if(rst == 1) begin
            ...
            begin
                if(jalr==1 || br==1) begin //jalr,br到达E时，要优先于jal处理, 因为考虑jalrE, jalD连续的情况
                ...
                else if(wb_select==1 && (reg1_srcD == reg_dstE || reg2_srcD == reg_dstE)) begin //LW应放在JAL之后处理，因为信号值互相覆盖的情况只有LWE，JALD的情况，但是JALD不会和LWE有任何的冲突，所以JALD相当于天然的bubble，此时不需要对LWE STALL
                    flushE=1;               //下一周期F段加载新指令，DF stall一个周期，E清空，防止读入D
                    flushD=0;
                    bubbleD=1;
                    bubbleF=1;
                end
